...
async function createOpsLogDb(notion, parentId) {
  return await notion.databases.create({
    ...
    title: [{ type: 'text', text: { content: '📋 Icarus Operations Log' } }],
    properties: {
      'Action': { title: {} },
      'Date': { date: {} },
      'Status': {
...
async function createWorkflowDb(notion, parentId) {
  return await notion.databases.create({
    parent: { type: 'page_id', page_id: parentId },
    title: [{ type: 'text', text: { content: '⚙️ Workflow Tracker' } }],
    properties: {
      'Task': { title: {} },
      'Status': {
        select: {
...
async function createPerformanceDb(notion, parentId) {
  return await notion.databases.create({
    parent: { type: 'page_id', page_id: parentId },
    title: [{ type: 'text', text: { content: '📊 Icarus Performance' } }],
    properties: {
      'Week': { title: {} },
      'Tasks Completed': { number: {} },
      'Tasks Failed': { number: {} },
      'Verifications Sent': { number: {} },
      'Verifications Approved': { number: {} },
      'Capability Gaps Identified': { number: {} },
      'Capability Gaps Resolved': { number: {} },
...
async function logToNotion({ action, outcome, status, category, nextStep, apiCostImpact }) {
  try {
    const notion = getClient();
    ...
    await notion.pages.create({
      ...
      properties: {
        'Action': { title: [{ text: { content: action || 'Unnamed action' } }] },
        'Date': { date: { start: new Date().toISOString() } },
        'Status': { select: { name: status || '🟢 Complete' } },
        'Category': { select: { name: category || 'System' } },
        'Outcome': { rich_text: [{ text: { content: outcome || '' } }] },
        'Next Step': { rich_text: [{ text: { content: nextStep || '' } }] },
        'API Cost Impact': { select: { name: apiCostImpact || 'Low' } },
      },
    });
...
async function createWorkflowTask({ task, status, priority, owner, dueDate, notes }) {
  try {
    const notion = getClient();
    ...
    const props = {
      'Task': { title: [{ text: { content: task || 'Unnamed task' } }] },
      'Status': { select: { name: status || '🟡 In Queue' } },
      'Priority': { select: { name: priority || '🟡 Medium' } },
      'Owner': { select: { name: owner || 'Icarus' } },
    };
